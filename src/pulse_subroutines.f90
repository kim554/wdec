...
9006  format(10x,'white dwarf sequence = Tc',i6,1x,'MLT alpha',F4.1/,10x, &
...
9903 format(5x,'l',2x,'k',2x,'period',3x, &
    ...
    if ( pulsoutput ) then
       write(51,9006) sequen,aml,idnint(teff)
...
2115 format('# -log(1-Mr/M*)    X_O        X_C        X_He       X_H', &
    ...
    do l=2,np
       if (newledoux) then
          write(63,2107) partt(l), mr(l), xheli(l),bled_new(l),xoxyg(l), &
...
!Subroutines
    use utils_subroutines, only : spline, splint
...
!!$
!!$    do j=1,4
!!$       ysub(1:nsm) = xcomp_orig(imin:nshint-1,j)
!!$       ysub(nsm+1:nsub) = xcomp_orig(nshint+1:imax,j)
...
!!$       xcomp_orig(nshint,j) = interp_value
!!$    end do
