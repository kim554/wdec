...
!Common blocks    
    ...
    integer :: i,npoints
    real*8, dimension(ndimo) ::  ams,corat
    real*8 :: amr,x2,efac,a
    real*8, dimension(ndimo-1) :: am
    real*8, dimension(ndimo) :: dm !ndimo-2
...
! Common blocks
    ...
    integer :: i,i0,i1
    real*8, dimension(ndimo) ::  xvals, yvals
...
! Find points from input profile on either side of point of interest
    do i=1,ndimo
       if ( (xvals(i) .gt. amr) .or. (i .eq. ndimo) ) then
          i1 = i
          exit 
       end if
    end do
