subroutine calculate_periods(inputdata)
...
!Common blocks
  ...
  use startmod
  use terp, only: stpms
  use xcompp, only: core_points

  implicit none

  real*8, dimension(36), intent(in) :: inputdata
  real*8 :: M_x, xtal, sigma, ff, best
  real*8 :: best_sigma
  real*8, parameter :: tiny1 = 1.0d-2, tiny2 = 1.0d-3, oxygen_core_edge = 9.9d-1
...
! Set input parameters
  ...
  Teff = inputdata(1)
  SM_star = inputdata(2)/1000.
  M_env = inputdata(3)/100.
  M_he = inputdata(4)/100.
  M_h = inputdata(5)/100.
!  M_h = MAX(inputdata(5)/100., M_he+2.0)
  xhebar = inputdata(6)/100.
  alph1 = inputdata(7)
  alph2 = inputdata(8)

! This is for the oxygen profile
  core_points(1,1) = 0.0d0
  core_points(1,2) = inputdata(20)/100.
  core_points(2,1) = inputdata(23)/100
  core_points(2,2) = core_points(1,2)
  core_points(3,1) = (inputdata(23)+inputdata(25))/100.
  core_points(3,2) = inputdata(21)/100.
  core_points(4,1) = core_points(3,1)+inputdata(26)/100.
  core_points(4,2) = inputdata(22)/100.
  if (core_points(4,1) .ge. oxygen_core_edge) then
     print *, "The oxygen profile goes too far out. The sum of the & 
          width parameters needs to be less than 99."
     stop
  end if
! Limit oxygen to reside in core part of the code  
! core_points(5,1) = 10.0d0**inputdata(19)-tiny1
! Do not limit oxygen to be in core part (with MESA EOS we're free!!!)  
!  core_points(5,1) = 1.0d0-tiny1
! Use w4 as a free parameter
  core_points(5,1) = core_points(4,1)+inputdata(27)/100
  if (core_points(5,1) .ge. oxygen_core_edge) then
     core_points(5,1) = oxygen_core_edge
  end if
  core_points(5,2) = tiny2
...
!inputdata(10) not used
!inputdata(11) not used
  permin = inputdata(12)
  permax = inputdata(13)
  lmax = INT(inputdata(14))
  nper = INT(inputdata(15))
  neuts = INT(inputdata(16))
!inputdata(17) not used
  maxion = inputdata(18)
  stpms = inputdata(19)
  tnot = INT(inputdata(24))
